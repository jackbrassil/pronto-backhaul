...
#right wifi nodee is configured as AP with interface wlp3s0 and IP 10.42.0.1
RIGHT_IFACE1=enp1s0            ### interface of packets arriving to right node
#RIGHT_IFACE2=wlp3s0            ### interface of packets arriving to right node (wifi)
LEFT_SRC1=10.250.156.56         ### IP of left source of packets arriving
LEFT_SRC2=10.42.0.101         ### IP of left source of packets arriving (wifi)
...
sudo iptables -t nat -A PREROUTING -s $LEFT_SRC2 -p tcp -i $RIGHT_IFACE2 --dport 12020 -j CONNMARK --set-mark 0x3
